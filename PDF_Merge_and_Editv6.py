...
def mergePages(): #entry, window):
    ...
    listvar.set(filesG)
    listbox=tk.Listbox(mergeWindow, height=10, width=20, listvariable=listvar, font="arial 12").grid()
    # this button is used to invoke get_item function
    # tk.Button(root, text='Сшить только выбранные!', command=merge_items(listbox, originalDir, mergeWindow), font="arial 12 bold").grid()
    tk.Button(mergeWindow, text="Искать...", command=lambda entry=folderPicker(originalDir, mergeWindow): get_pages(entry)).grid(row=1, column=2, pady=5, padx=5, sticky=stickyFill)
    tk.Button(mergeWindow, text="Сшить!", command=merge_items(listbox, originalDir, mergeWindow, progress)).grid(row=2, column=0, columnspan=3, padx=5, pady=10, sticky=stickyFill)
...
# body of GUI
my_img1 = tk.PhotoImage(file = "D:\\users\\gorodetsky\\GIT\\PDF_Merge_and_Edit\\resources\\SK.png")
my_img = tk.PhotoImage(file = "D:\\users\\gorodetsky\\GIT\\PDF_Merge_and_Edit\\resources\\logoSK.png")
...
tk.Label(selector, text="Работа с PDF v 0.5.0").grid(row=1, column=1, pady=5, padx=5)
#nfiles = tk.IntVar()
